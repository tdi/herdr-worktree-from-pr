...
export function formatLine(pr) {
  ...
  // Author sits right after the number so it stays visible even when a long
  // title would otherwise push it off the right edge of the picker overlay.
  return `#${pr.number}  @${pr.authorLogin}  ${pr.title}  (${pr.headRefName})${fork}`;
}
